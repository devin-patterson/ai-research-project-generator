...
class MethodologySignature(dspy.Signature):
    ...
    data_collection_methods: list[str] = dspy.OutputField(
        desc="Recommended data collection methods"
    )
...
class MethodologyRecommenderModule(dspy.Module):
    ...
    def forward(
        self, topic: str, research_type: str, discipline: str, academic_level: str
    ) -> dspy.Prediction:
...
def optimize_topic_analyzer(
    ...
    # Create optimizer
    optimizer = dspy.MIPROv2(metric=topic_analysis_metric, auto=optimization_level, num_threads=4)

    # Optimize
    optimized = optimizer.compile(
        module, trainset=trainset, max_bootstrapped_demos=2, max_labeled_demos=2
    )
...
def optimize_paper_synthesizer(
    ...
    # Create optimizer
    optimizer = dspy.MIPROv2(metric=paper_synthesis_metric, auto=optimization_level, num_threads=4)
...
def load_optimized_module(module_class: type, path: str) -> dspy.Module:
    """
