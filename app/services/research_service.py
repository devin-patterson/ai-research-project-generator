...
import sys
sys.path.insert(0, '.')
from llm_provider import LLMConfig, LLMProvider, ResearchLLMAssistant, create_llm_client
...
class ResearchService:
    ...
    async def generate_project(self, request: ResearchRequest) -> ResearchResponse:
        ...
        try:
            ...
            if request.search_papers and self._academic_search:
                ...
                discovered_papers, paper_statistics, search_sources = await self._search_papers(request)
            ...
            response = ResearchResponse(
                ...
                discovered_papers=[
                    PaperSchema(**p) for p in discovered_papers
                ] if discovered_papers else None,
    ...
    def _validate_project(self, base_project: dict, context: ResearchContext) -> ValidationReportSchema:
        ...
        return ValidationReportSchema(
            ...
            issues=[{"level": i.level.value, "title": i.title, "description": i.description, "category": i.category.value} 
                   for i in report.issues],
            recommendations=report.recommendations,
            enhancement_opportunities=report.enhancement_opportunities,
        )
    
    async def _search_papers(self, request: ResearchRequest) -> Tuple[list, Optional[PaperStatistics], list]:
