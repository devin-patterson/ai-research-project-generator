...
class AIResearchEngine:
        ...
        if self.use_llm and self.llm_assistant:
            ...
            try:
                ...
                if discovered_papers:
                    ...
                    synthesis = self.llm_assistant.synthesize_findings(
                        paper_dicts, research_question
                    )
    ...
    def export_enhanced_project(
        self, project: EnhancedResearchProject, format: str = "markdown"
    ) -> str:
...
"""
            ...
            sorted_papers = sorted(
                project.discovered_papers, key=lambda p: p.get("citation_count") or 0, reverse=True
            )[:10]
