...
class CitationFormatter:
    ...
    def _format_apa(citation: Citation) -> str:
        ...
        # Title
        if citation.title:
            if citation.publication_type in [PublicationType.BOOK, PublicationType.REPORT, PublicationType.THESIS]:
            ...
            for i, author in enumerate(citation.authors[:3]):
                if "," in author:
                    ...
                    if len(author_parts) >= 2:
                        formatted_authors.append(f"{author_parts[-1]}, {' '.join(author_parts[:-1])}")
                    else:
                        formatted_authors.append(author)
...
class CitationManager:
    ...
    def generate_bibliography(
        ...
        if sort_by == "author":
            sorted_keys = sorted(
                self.citations.keys(),
                key=lambda k: (self.citations[k].authors[0] if self.citations[k].authors else "ZZZ").lower()
            )
        elif sort_by == "year":
            sorted_keys = sorted(
                self.citations.keys(),
                key=lambda k: self.citations[k].year or 9999
            )
        else:  # order
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "CitationManager":
        ...
        manager = cls(
            default_style=CitationStyle(data.get("default_style", "apa7"))
        )
