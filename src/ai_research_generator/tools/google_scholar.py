...
class GoogleScholarTool:
    ...
    async def search(
        ...
        if SCHOLARLY_AVAILABLE:
            try:
                return await self._search_scholarly(query, num_results, year_from, year_to, include_citations)
            except Exception as e:
    ...
    async def _search_serpapi(
        ...
        for item in data.get("organic_results", []):
            ...
            results.append(GoogleScholarResult(
                ...
                venue=item.get("publication_info", {}).get("summary", "").split(" - ")[-1] if item.get("publication_info") else None,
