...
class FactVerificationResult(BaseModel):
    ...
    verdict: str = Field(
        description="Verification verdict (verified/unverified/disputed/insufficient_evidence)"
    )
...
class WebSearchTool(BaseTool):
    ...
    async def _search_duckduckgo(self, query: str, num_results: int) -> List[WebSearchResult]:
        ...
        response = await self.http_client.get(
            "https://api.duckduckgo.com/", params={"q": query, "format": "json", "no_html": 1}
        )
        response.raise_for_status()
        data = response.json()
...
class AcademicSearchTool(BaseTool):
    ...
    async def _search_openalex(
        ...
        params = {"search": query, "per_page": min(num_results, 50), "sort": "cited_by_count:desc"}
    ...
    async def _search_crossref(
        ...
        params = {"query": query, "rows": min(num_results, 50), "sort": "relevance"}
...
class FactVerificationTool(BaseTool):
    ...
    async def execute(self, claim: str, context: Optional[str] = None) -> FactVerificationResult:
        """
        ...
        try:
            ...
            web_results = await web_tool.execute(query=f"fact check: {claim}", num_results=5)

            # Search academic sources
            academic_results = await academic_tool.execute(query=claim, num_results=5)

            # Combine sources
            all_sources = []
            for r in web_results:
                all_sources.append(
                    {"type": "web", "title": r.title, "content": r.snippet, "url": r.url}
                )
    ...
    async def _analyze_evidence(
        self, claim: str, context: Optional[str], sources: List[Dict[str, Any]]
    ...
    async def conduct_research(
        ...
        if include_web_search:
            ...
            web_results = await self.web_search.execute(
                query=f"{topic} {research_question}", num_results=self.config.max_web_results
            )
            for r in web_results:
                all_sources.append(
                    {
        ...
        if include_academic_search:
            ...
            academic_results = await self.academic_search.execute(
                query=topic, num_results=self.config.max_academic_results
            )
            for r in academic_results:
                all_sources.append(
                    {
    ...
    async def _synthesize():
        ...
        try:
            ...
            result = await tool.execute(
                topic=topic, sources=sources, research_question=research_question or None
            )
...
def generate_research_report(
    topic: str, research_question: str, additional_context: str = ""
) -> str:
    """
